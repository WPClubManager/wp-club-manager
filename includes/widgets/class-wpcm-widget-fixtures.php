...
class WPCM_Fixtures_Widget extends WP_Widget {
	 ...
	 * @return void
	 */
	function WPCM_Fixtures_Widget() {

		/* Widget variable settings. */
		$this->wpcm_widget_cssclass = 'wpcm-widget widget-fixtures';
		$this->wpcm_widget_description = __( 'Display upcoming fixtures.', 'wpclubmanager' );
		$this->wpcm_widget_idbase = 'wpcm-fixtures-widget';
		$this->wpcm_widget_name = __( 'WPCM Fixtures', 'wpclubmanager' );

		/* Widget settings. */
		$widget_ops = array( 'classname' => $this->wpcm_widget_cssclass, 'description' => $this->wpcm_widget_description );

		/* Create the widget. */
		$this->WP_Widget('wpcm_fixtures', $this->wpcm_widget_name, $widget_ops);
	}
	...
	function widget( $args, $instance ) {
		...
		echo '<div class="wpcm-matches-widget clearfix">';
		echo '<ul>';
		if ( $size > 0 ) {
			foreach( $matches as $match ) {
				...
				$seasons = get_the_terms( $match->ID, 'wpcm_season' );
	...
	function form( $instance ) {
		...
		$defaults = array(
			...
			'club' => get_option( 'wpcm_default_club' ),
			'venue' => null,
			'linktext' => __( 'View all fixtures', 'wpclubmanager' ),
			'linkpage' => null,
			'title' => __( 'Fixtures', 'wpclubmanager' ),
			'show_date' => null,
			'show_time' => null,
			'show_comp' => null,
			'show_team' => null
